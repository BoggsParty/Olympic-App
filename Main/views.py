...
def view_all(request):
    ...
    mens_all_around_gymnastics = mens_all_around_gymnasticsSelection.objects.filter(Q(sport_name='mens-all-around-gymnastics') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_track_4x100_relay = womens_track_4x100_relaySelection.objects.filter(Q(sport_name='womens-track-4x100-relay') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_track_4x100_relay = mens_track_4x100_relaySelection.objects.filter(Q(sport_name='mens-track-4x100-relay') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_decathalon = womens_decathalonSelection.objects.filter(Q(sport_name='womens-decathalon') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_decathalon = mens_decathalonSelection.objects.filter(Q(sport_name='mens-decathalon') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_swimming_4x100_medley_relay = womens_swimming_4x100_medley_relaySelection.objects.filter(Q(sport_name='womens-swimming-4x100-medley-relay') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_swimming_4x100_medley_relay = mens_swimming_4x100_medley_relaySelection.objects.filter(Q(sport_name='mens-swimming-4x100-medley-relay') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_swimming_200m_backstroke = womens_swimming_200m_backstrokeSelection.objects.filter(Q(sport_name='womens-swimming-200m-backstroke') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_swimming_1500m_freestyle = mens_swimming_1500m_freestyleSelection.objects.filter(Q(sport_name='mens-swimming-1500m-freestyle') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_golf = mens_golfSelection.objects.filter(Q(sport_name='mens-golf') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_soccer = womens_soccerSelection.objects.filter(Q(sport_name='womens-soccer') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    ...
    womens_beach_volleyball = womens_beach_volleyballSelection.objects.filter(Q(sport_name='womens-beach-volleyball') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_waterpolo = mens_waterpoloSelection.objects.filter(Q(sport_name='mens-waterpolo') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_bmx = womens_bmxSelection.objects.filter(Q(sport_name='womens-bmx') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_bmx = mens_bmxSelection.objects.filter(Q(sport_name='mens-bmx') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    mens_handball = mens_handballSelection.objects.filter(Q(sport_name='mens-handball') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('-id')[0]
    womens_basketball = womens_basketballSelection.objects.filter(Q(sport_name='womens-basketball') | Q(sport_name='none')).filter(Q(user=request.user) | Q(user__isnull=True)).order_by('id')[0]
